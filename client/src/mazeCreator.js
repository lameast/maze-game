...
const mazeCreator = (size) => {
    ...
    const maze = new Array(mazeSize).fill().map(() => new Array(mazeSize).fill(0)); //Initialise maze
    const stack = [[1,1]];
    const directions = [[-2, 0], [0, -2], [2, 0], [0, 2]]; //left, down, right, up
    maze[1][1] = 1;

    while (stack.length > 0){
        let currentPosition = stack.pop();
        const shuffledArray = shuffleArray(directions);
        //Check cell in random direction
        for(let i = 0; i < shuffledArray.length; i++){
            ...
            if (x > 0 && x < mazeSize-1 && y > 0 && y < mazeSize-1 && maze[x][y] === 0){
                //If unvisited, visit this cell
                stack.push(currentPosition);
                maze[x][y] = 1; //Mark cell as visited
                if(dirX > 0){
                    maze[x - 1][y] = 1;
                }else if (dirX < 0){
                    maze[x + 1][y] = 1;
